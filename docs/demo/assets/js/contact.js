...
(function () {
  ...
  if (!vehicleRaw) { window.location.replace("index.html"); return; }
  if (!damagesRaw || Object.keys(JSON.parse(damagesRaw) || {}).length === 0) {
    window.location.replace("selector.html"); return;
  }
  if (!serviceRaw) { window.location.replace("service.html"); return; }
  ...
  backBtn.addEventListener("click", () => { window.location.href = "service.html"; });
  ...
  reviewBody.addEventListener("click", (e) => {
    ...
    closeReviewModal();
    if (step === "vehicle") { window.location.href = "index.html";    return; }
    if (step === "damages") { window.location.href = "selector.html"; return; }
    if (step === "service") { window.location.href = "service.html";  return; }
  ...
  function sendPayload(payload) {
    ...
    // DEMO MODE: there is no PHP backend on GitHub Pages. We fake a successful
    // response after a short delay so the success modal + reference ID flow
    // still demonstrates correctly. The actual payload is logged to the
    // console so a curious dev can inspect what would have been sent.
    new Promise((resolve) => {
